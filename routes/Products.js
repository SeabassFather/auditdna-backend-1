...
router.get('/catalog/:sku', async (req, res) => {
  try {
    const r = await pool.query('SELECT * FROM product_catalog WHERE sku = $1', [req.params.sku]);
...
router.get('/categories', async (req, res) => {
  try {
    const r = await pool.query('SELECT category, COUNT(*) as count FROM product_catalog GROUP BY category ORDER BY count DESC');
...
router.get('/origins', async (req, res) => {
  try {
    const r = await pool.query('SELECT origin_country, COUNT(*) as count FROM product_catalog GROUP BY origin_country ORDER BY count DESC');
    res.json({ success: true, data: r.rows });
  } catch (e) { res.status(500).json({ success: false, error: e.message }); }
});
...
router.get('/manifests/:id/items', async (req, res) => {
  try {
    const r = await pool.query('SELECT * FROM manifest_items WHERE manifest_id = $1 ORDER BY id', [req.params.id]);
    res.json({ success: true, data: r.rows });
  } catch (e) { res.status(500).json({ success: false, error: e.message }); }
});
...
router.get('/price-alerts', async (req, res) => {
  try {
    const r = await pool.query('SELECT * FROM price_alerts WHERE active = true ORDER BY created_at DESC');
    res.json({ success: true, data: r.rows });
  } catch (e) { res.status(500).json({ success: false, error: e.message }); }
});
...
router.get('/health', async (req, res) => {
  try {
    ...
    const products = await pool.query('SELECT COUNT(*) FROM product_catalog');
    const growers = await pool.query('SELECT COUNT(*) FROM growers');
    const buyers = await pool.query('SELECT COUNT(*) FROM buyers');
    const categories = await pool.query('SELECT COUNT(DISTINCT category) FROM product_catalog');
    const origins = await pool.query('SELECT COUNT(DISTINCT origin_country) FROM product_catalog');
    res.json({
      success: true,
