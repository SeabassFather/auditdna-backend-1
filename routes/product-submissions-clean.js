...
router.get('/', async (req, res) => {
  ...
  try {
    ...
    const r = await global.db.query('SELECT * FROM product_submissions ORDER BY submitted_at DESC LIMIT $1 OFFSET $2',[limit,offset]);
...
router.post('/:id/approve', async (req, res) => {
  const pool = req.app.locals.pool;
  try {
    const r = await global.db.query(`UPDATE product_submissions SET status='approved', admin_note=$1, approved_by=$2, approved_at=NOW() WHERE id=$3 RETURNING *`,[req.body.note||null,req.body.approvedBy||'admin',req.params.id]);
...
router.post('/:id/reject', async (req, res) => {
  const pool = req.app.locals.pool;
  try {
    const r = await global.db.query(`UPDATE product_submissions SET status='rejected', admin_note=$1, rejected_by=$2, rejected_at=NOW() WHERE id=$3 RETURNING *`,[req.body.note||null,req.body.rejectedBy||'admin',req.params.id]);
    res.json({ success: true, submission: r.rows[0] });
  } catch(e) { res.status(500).json({ error: e.message }); }
});
