...
router.post('/trade-credit', async (req, res) => {
  try {
    ...
    try { await pool.query("INSERT INTO brain_events (event_type, module, payload, created_at) VALUES ($1, $2, $3, NOW())", ['TRADE_CREDIT_APP', 'credit-apps', JSON.stringify(req.body)]); } catch(e) {}
...
router.post('/factoring', async (req, res) => {
  try {
    ...
    try { await pool.query("INSERT INTO brain_events (event_type, module, payload, created_at) VALUES ($1, $2, $3, NOW())", ['FACTORING_APP', 'credit-apps', JSON.stringify(req.body)]); } catch(e) {}
...
router.post('/net-terms', async (req, res) => {
  try {
    ...
    try { await pool.query("INSERT INTO brain_events (event_type, module, payload, created_at) VALUES ($1, $2, $3, NOW())", ['NET_TERMS_APP', 'credit-apps', JSON.stringify(req.body)]); } catch(e) {}
...
router.patch('/:id/status', async (req, res) => {
  try {
    ...
    try { await pool.query("INSERT INTO brain_events (event_type, module, payload, created_at) VALUES ($1, $2, $3, NOW())", ['CREDIT_APP_STATUS_UPDATE', 'credit-apps', JSON.stringify({id: req.params.id, status})]); } catch(e) {}
