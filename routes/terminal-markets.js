...
router.get('/shipments', async (req, res) => {
  try {
    const r = await termglobal.db.query('SELECT * FROM shipments ORDER BY created_at DESC LIMIT 100').catch(()=>({rows:[]}));
...
router.post('/shipments', async (req, res) => {
  try {
    ...
    const r = await termglobal.db.query(
    ...
    try { termglobal.db.query("INSERT INTO brain_events (event_type, module, payload, created_at) VALUES ($1,$2,$3,NOW())", ['SHIPMENT_CREATED','terminal-markets',JSON.stringify(req.body)]).catch(()=>{}); } catch(e) {}
