...
async function initTable(pool) {
  try {
    await pool.query('CREATE TABLE IF NOT EXISTS product_submissions (id SERIAL PRIMARY KEY, grower_name TEXT, grower_email TEXT, grower_phone TEXT, company TEXT, commodity TEXT, variety TEXT, quantity INTEGER, unit TEXT, pack_size TEXT, fob_price NUMERIC(10,2), available_date DATE, shelf_life INTEGER, harvest_location TEXT, destination_market TEXT, quality_grade TEXT, notes TEXT, status TEXT DEFAULT \'pending\', admin_note TEXT, approved_by TEXT, approved_at TIMESTAMPTZ, rejected_by TEXT, rejected_at TIMESTAMPTZ, submitted_at TIMESTAMPTZ DEFAULT NOW())');
...
router.get('/', async (req,res) => {
  try {
    ...
    const r = await pool.query('SELECT * FROM product_submissions ORDER BY submitted_at DESC LIMIT  OFFSET ',[limit,offset]);
...
router.post('/:id/approve', async (req,res) => {
  try {
    const pool = req.app.locals.pool;
    const r = await pool.query("UPDATE product_submissions SET status='approved',admin_note=,approved_by=,approved_at=NOW() WHERE id= RETURNING *",[req.body.note||null,req.body.approvedBy||'admin',req.params.id]);
...
router.post('/:id/reject', async (req,res) => {
  try {
    const pool = req.app.locals.pool;
    const r = await pool.query("UPDATE product_submissions SET status='rejected',admin_note=,rejected_by=,rejected_at=NOW() WHERE id= RETURNING *",[req.body.note||null,req.body.rejectedBy||'admin',req.params.id]);
    res.json({success:true,submission:r.rows[0]});
  } catch(e){res.status(500).json({error:e.message});}
});
