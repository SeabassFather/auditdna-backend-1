...
// Auth middleware
let authRequired;
try { authRequired = require('../src/Auth').authRequired; }
catch (e) { try { authRequired = require('./auth').authRequired; } catch (e2) { authRequired = (req, res, next) => next(); } }
...
router.post('/score/:dealId', authRequired, async (req, res) => {
  ...
  try {
    ...
    console.log('[FACTOR] Score complete - primary=' + result.scoring.primary_recommendation?.partner_id);
...
// Body: { deal_id, partner_id, draft (optional - will auto-draft if missing), dryRun (optional) }
router.post('/send', authRequired, async (req, res) => {
  const pool = getPool(req);
  try {
    const { deal_id, partner_id, dryRun } = req.body || {};
