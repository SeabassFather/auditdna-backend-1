...
// AUTONOMOUS EMAIL BLAST ENGINE — 8 AI AGENTS
...
const SYSTEM_PROMPTS = {
  AVOCADO: `You are the Avocado Program Agent for Mexausa Food Group, Inc. You write professional, direct outreach emails for the year-round Hass avocado program. Saul Garcia sources from Michoacan, Jalisco, Nayarit. Sizes: 32ct-70ct. Year-round supply. Full SENASICA/USDA compliance. No emojis. No fluff. Direct, professional, produce-industry tone. Always include: commodity specs, availability, compliance status, and call to action. Sign as Saul Garcia, Mexausa Food Group. Return ONLY the email body — no subject line, no preamble.`,
...
// ─── SUBJECTS ────────────────────────────────────────────────────────────────
const SUBJECTS = {
  AVOCADO:           'Year-Round Hass Avocado Supply — Direct Source Michoacan | Mexausa Food Group',
  BUYER_OUTREACH:    'Direct Grower Access — Fresh Produce Network | LOAF Platform',
...
async function fireBrainEvent(brain, eventType, payload) {
  try {
    if (brain && typeof brain.emit === 'function') {
      ...
      // brain in direct-call mode — log only
...
async function sendEmail(transporter, to, subject, body, fromName) {
  return transporter.sendMail({
    from: `"${fromName || 'Saul Garcia — Mexausa Food Group'}" <sgarcia1911@gmail.com>`,
    to,
    subject,
    text: body,
...
Include: operational status, email blast performance, pipeline health, recommendations for next cycle.`;
    ...
    await sendEmail(
      ...
      SUBJECTS.PRODUCTION_REPORT + ` — ${new Date().toLocaleString()}`,
      reportText,
...
async function runAgent(app, brain, agentId) {
  ...
  try {
    ...
    if (contacts.length === 0) {
      console.log(`[AGENT-${agentId}] No contacts found — skipping blast`);
      await logRun(pool, runId, agentId, 'skipped', stats, 'No contacts available');
      return;
    }

    // Generate AI email body — one body per run, personalized at send time
...
Target segment: ${agent.category} — commodities including ${contacts.slice(0,5).map(c=>c.commodity||'fresh produce').join(', ')}.
...
Make it specific, actionable, and professional. No emojis.`;
    ...
    console.log(`[AGENT-${agentId}] Run complete — sent: ${stats.sent} | failed: ${stats.failed} | targeted: ${stats.targeted}`);
...
// ─── INCOMING EMAIL INQUIRY HANDLER ──────────────────────────────────────────
// Handles inbound inquiries — routes to correct agent, generates AI reply, logs
async function handleInboundInquiry(app, brain, { from, subject, body, agent_hint }) {
  ...
  try {
    ...
    // Send reply
    await sendEmail(transporter, from, `Re: ${subject}`, replyBody, 'Saul Garcia — Mexausa Food Group');
...
function startAutonomousAgents(app, brain) {
  ...
  // Stagger initial runs — don't blast all at once on startup
  agentList.forEach((agentId, idx) => {
    ...
    console.log(`[AUTONOMOUS] ${agent.name} scheduled — first run in ${Math.round(initialDelay/60000)}min, then every ${Math.round(agent.schedule_ms/3600000)}hr`);
  });

  // Production report every 2 hours — first run after 5 minutes
  setTimeout(() => {
    runProductionReport(app, brain);
    setInterval(() => runProductionReport(app, brain), AGENTS.PRODUCTION_REPORT.schedule_ms);
  }, 5 * 60 * 1000);

  console.log('[AUTONOMOUS] Production report agent scheduled — first run in 5min, then every 2hr');
