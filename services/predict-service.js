...
async function predictCrop({ pool, grower, commodity, acres, planting_date, expected_yield_per_acre, region, variety, organic }) {
  ...
  const dealInsert = await pool.query(
    ...
    [grower.id, commodity, volumeLbs, prediction.estimated_total_revenue || prediction.predicted_margin || 0, yield_id]
  );
...
module.exports = { predictCrop, draftCourtesyLetter, findMatchedBuyers, MODEL_PREDICT, MODEL_LETTER };