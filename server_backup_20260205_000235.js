...
console.log('\nðŸ” Discovering routes...\n');

function discoverRoutes(dir, basePath = '/api') {
  if (!fs.existsSync(dir)) {
    console.log(`âš ï¸  Routes directory not found: ${dir}`);
    return;
  }
  ...
  files.forEach(file => {
    ...
    try {
      ...
      console.log(`âœ… Loaded: ${routePath.padEnd(40)} ${filePath}`);
    } catch (error) {
      failedRoutes.push({ path: routePath, file: filePath, error: error.message });
      console.log(`âŒ Failed: ${routePath.padEnd(40)} ${error.message}`);
    }
  });
}
...
console.log('\nðŸ”§ Loading manual routes...\n');

manualRoutes.forEach(({ path: routePath, file, name }) => {
  try {
    const route = require(file);
    app.use(routePath, route);
    console.log(`âœ… Loaded: ${name.padEnd(25)} â†’ ${routePath}`);
  } catch (error) {
    console.log(`âš ï¸  Skipped: ${name.padEnd(25)} (${error.message.split('\n')[0]})`);
  }
});
...
console.log('\nðŸ“Š Loading Elite Analytics endpoints...\n');
...
app.post('/api/analytics/spartan_cases', (req, res) => {
  try {
    ...
    console.log(`âœ… Spartan300 case saved: ${caseData.category} - Recovery: $${caseData.recoveryAmount}`);
    ...
    console.error('âŒ Error saving Spartan case:', error);
...
app.post('/api/analytics/trojan_cases', (req, res) => {
  try {
    ...
    console.log(`âœ… Trojan700 case saved: ${caseData.category} - Savings: $${caseData.savingsAmount}`);
    ...
    console.error('âŒ Error saving Trojan case:', error);
    res.status(500).json({ error: error.message });
  }
});
...
app.post('/api/analytics/data_sales', (req, res) => {
  try {
    ...
    console.log(`âœ… Data sale saved: ${saleData.agency} - $${saleData.amount}`);
    ...
    console.error('âŒ Error saving data sale:', error);
    res.status(500).json({ error: error.message });
  }
});
...
app.get('/api/analytics/dashboard', (req, res) => {
  try {
    ...
    console.error('âŒ Error getting dashboard:', error);
    res.status(500).json({ error: error.message });
  }
});
...
app.delete('/api/analytics/clear', (req, res) => {
  ...
  console.log('ðŸ—‘ï¸  Analytics storage cleared');
...
console.log('âœ… Elite Analytics endpoints loaded');

// â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
// AI LEARNING ENDPOINTS (35 Cowboys - Inline Backup)
// â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•

console.log('\nðŸ¤  Loading AI Learning endpoints...\n');
...
if (!fs.existsSync(AI_LEARNING_PATH)) {
  ...
  console.log(`âœ… Created AI Learning data directory: ${AI_LEARNING_PATH}`);
}
...
app.post('/api/ai-learning/save', (req, res) => {
  try {
    ...
    console.log(`âœ… AI Learning session saved: ${fileName}`);
    ...
    console.error('âŒ Error saving AI learning data:', error);
    res.status(500).json({ error: error.message });
  }
});
...
console.log('âœ… AI Learning endpoints loaded');
// â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
...
console.log('\nðŸŒ± Loading Grower Intelligence endpoints...\n');
...
if (!fs.existsSync(UPLOADS_PATH)) {
  ...
  console.log(`âœ… Created uploads directory: ${UPLOADS_PATH}`);
}
...
app.get('/api/growers/stats', async (req, res) => {
  try {
    ...
    if (typeof pool !== 'undefined') {
      try {
        const totalResult = await global.db.query('SELECT COUNT(*) FROM growers');
        const activeResult = await global.db.query("SELECT COUNT(*) FROM growers WHERE status = 'active'");
        const pendingResult = await global.db.query("SELECT COUNT(*) FROM growers WHERE status = 'pending'");
        const tier0Result = await global.db.query('SELECT COUNT(*) FROM growers WHERE tier = 0');
        const tier1Result = await global.db.query('SELECT COUNT(*) FROM growers WHERE tier = 1');
        const tier2Result = await global.db.query('SELECT COUNT(*) FROM growers WHERE tier = 2');
        const tier3Result = await global.db.query('SELECT COUNT(*) FROM growers WHERE tier = 3');
        const complianceResult = await global.db.query('SELECT COUNT(*) FROM growers WHERE grs_score >= 75');
        ...
        console.log('âš ï¸  Database query failed, using defaults:', dbError.message);
      }
    }
    
    res.json(stats);
  } catch (error) {
    console.error('âŒ Error getting grower stats:', error);
    res.status(500).json({ error: error.message });
  }
});
...
app.post('/api/documents/upload', upload.single('file'), (req, res) => {
  try {
    ...
    console.log(`âœ… Document uploaded: ${docRecord.originalName} (${(docRecord.size / 1024).toFixed(2)} KB)`);
...
app.post('/api/ai/multi-platform', async (req, res) => {
  try {
    ...
    console.log(`âœ… Multi-AI analysis complete: ${overallConfidence.toFixed(4)} confidence`);
    ...
    res.json({
      ...
      recommendations: [
        ...
        'TraceSafeâ„¢ lot tracking enabled',
        'Buyer matching recommendations available'
      ]
    });
  } catch (error) {
    console.error('âŒ Error in multi-platform analysis:', error);
    res.status(500).json({ error: error.message });
  }
});
...
app.post('/api/qr/generate', (req, res) => {
  try {
    ...
    console.log(`âœ… QR code generated for grower: ${growerId}`);
    ...
    console.error('âŒ Error generating QR code:', error);
    res.status(500).json({ error: error.message });
  }
});
  ...
  try {
    ...
    if (growerIntelStorage.cowboysStatus[team]) {
      ...
      if (cowboy) {
        ...
        console.log(`âœ… Cowboy ${cowboyId} status updated to: ${status}`);
      }
    }
...
console.log('âœ… Grower Intelligence endpoints loaded');
...
server = app.listen(PORT, () => {
  ...
  console.log(`   - Health:      http://process.env.DB_HOST:${PORT}/health`);
  console.log(`   - Metrics:     http://process.env.DB_HOST:${PORT}/metrics`);
  console.log(`   - Routes:      http://process.env.DB_HOST:${PORT}/health/routes`);
  console.log(`   - API:         http://process.env.DB_HOST:${PORT}/api`);
  console.log('â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€');
  console.log('   ELITE ANALYTICS ENDPOINTS:');
  console.log(`   - Spartan300:  http://process.env.DB_HOST:${PORT}/api/analytics/spartan_cases`);
  console.log(`   - Trojan700:   http://process.env.DB_HOST:${PORT}/api/analytics/trojan_cases`);
  console.log(`   - Data Sales:  http://process.env.DB_HOST:${PORT}/api/analytics/data_sales`);
  console.log(`   - Dashboard:   http://process.env.DB_HOST:${PORT}/api/analytics/dashboard`);
  console.log('â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€');
  console.log('   AI LEARNING ENDPOINTS (35 Cowboys):');
  console.log(`   - Save:        http://process.env.DB_HOST:${PORT}/api/ai-learning/save`);
  console.log(`   - Stats:       http://process.env.DB_HOST:${PORT}/api/ai-learning/stats`);
  console.log(`   - Files:       http://process.env.DB_HOST:${PORT}/api/ai-learning/files`);
  console.log(`   - Email:       http://process.env.DB_HOST:${PORT}/api/email/send-learning-report`);
  console.log('â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€');
  console.log('   ðŸŒ± GROWER INTELLIGENCE ENDPOINTS:');
  console.log(`   - Stats:       http://process.env.DB_HOST:${PORT}/api/growers/stats`);
  console.log(`   - Upload:      http://process.env.DB_HOST:${PORT}/api/documents/upload`);
  console.log(`   - Multi-AI:    http://process.env.DB_HOST:${PORT}/api/ai/multi-platform`);
  console.log(`   - QR Generate: http://process.env.DB_HOST:${PORT}/api/qr/generate`);
  console.log(`   - Cowboys:     http://process.env.DB_HOST:${PORT}/api/cowboys/status`);
  console.log('â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•');
  ...
  console.log('â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•');
  console.log('   ðŸ¤  YEEHAW! AI COWBOYS ARE SADDLED UP! ðŸ¤ ');
  console.log('â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•');
  console.log('\n');
});
