...
router.post('/register', async (req, res) => {
  ...
  try {
    ...
    try {
      // ID-safe payload — NO name, RFC, banking passed between modules
      const safePayload = {
        type: 'GROWER_REGISTERED_DB',
        grower_id: grower.id,
        // IDENTITY: ID number only — no name/company/RFC
      ...
      // 3. Push to CRM contacts (SaulIntelCRM sees full profile via admin auth — grower_id link)
      pool.query(
