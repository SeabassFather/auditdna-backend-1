...
// Last Updated: 2025-11-16 21:13:49 UTC
...
app.get('/', (req, res) => {
  res.json({ 
    ...
    endpoints: {
      ...
      customers: [
        'GET  /api/customers/search?email=xxx - Search customer by email',
        'POST /api/customers/save - Create/update customer',
        'POST /api/customers/add-invoice - Add invoice to customer',
        'POST /api/customers/record-payment - Record payment',
        'GET  /api/customers/list - List all customers'
      ],
      manifests: [
        'GET  /api/manifests/list - List all manifests',
        'GET  /api/manifests/inventory/realtime - Get real-time inventory'
      ],
      produce: [
  ...
  try {
    ...
    for (const item of lineItems) {
      ...
      if (product.currentStock < qty) {
        throw new Error(`Insufficient stock for ${product.productName}. Available: ${product.currentStock}, Requested: ${qty}`);
      }
    }
...
app.listen(PORT, () => {
  ...
  console.log('[TEST] USDA: GET http://process.env.DB_HOST:' + PORT + '/api/produce/usda/search/lettuce');
  console.log('============================================================');
});
